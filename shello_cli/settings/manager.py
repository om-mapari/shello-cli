...
class SettingsManager:
    ...
    def _parse_remote_server_config(self, remote_data: Optional[Dict[str, Any]]) -> Optional[RemoteServerConfig]:
        """Parse Remote Server configuration.
 
        Args:
            remote_data: Dictionary with Remote Server config or None
 
        Returns:
            RemoteServerConfig or None if not configured
        """
        ...
        return RemoteServerConfig(
            ...
            username=remote_data.get('username') or remote_data.get('user'),
            password=remote_data.get('password'),
            private_key_path=remote_data.get('private_key_path') or remote_data.get('key_path') or remote_data.get('key'),
            su_password=remote_data.get('su_password') or remote_data.get('suPassword'),
            sudo_password=remote_data.get('sudo_password') or remote_data.get('sudoPassword'),
            disable_sudo=remote_data.get('disable_sudo') or remote_data.get('disableSudo'),
            timeout=remote_data.get('timeout'),
        )
    ...
    def get_remote_server_config(self) -> Optional[RemoteServerConfig]:
        """Get merged Remote Server configuration. Project overrides user settings."""
        user_settings = self.load_user_settings()
        project_settings = self.load_project_settings()
        
        # Start with empty/default
        merged = RemoteServerConfig()
        
        # Check if user settings or project settings have remote server config
        has_config = False
        
        # Apply user settings if present
        if user_settings.remote_server:
            has_config = True
            for field_name in merged.__dataclass_fields__:
                val = getattr(user_settings.remote_server, field_name)
                if val is not None:
                    setattr(merged, field_name, val)
                    
        # Apply project settings if present (overrides user settings)
        if project_settings.remote_server:
            has_config = True
            for field_name in merged.__dataclass_fields__:
                val = getattr(project_settings.remote_server, field_name)
                if val is not None:
                    setattr(merged, field_name, val)
