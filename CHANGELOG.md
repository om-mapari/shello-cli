...
### Fixed
- **Persistent PowerShell Shell Output**: Improved output sanitization, stripping carriage returns, and command echo matching logic for persistent shell execution on Windows.
...
### Added
- **Selection-Based Command Approval**: Replaced text-based `[a/d/custom]` input confirmation with an interactive selection menu using arrow keys/shortcuts via `prompt_toolkit`.
- **Keyboard Navigation**: Added interactive selection to AI provider setup (`shello setup`) and AWS credentials configuration.

### Changed
- **Cleaner CLI Experience**: Added automated cursor/scrollback clearing of selection menus, history pickers, and command confirmation menus upon choice.

## [0.8.2] - 2026-06-13

### Added
- **Remote CLI Execution Tool**: Native `run_remote_command` tool to execute shell commands on a remote server using SSH by wrapping the Node.js `ssh-mcp` server.
- **SSH Connection Configuration**: First-class `ssh` configuration block in `.shello/settings.yml` (project-level) and `~/.shello_cli/user-settings.yml` (global).
- **Command Trust & Safety**: Full safety checks for remote commands using local trust policies and warning dialogs.
- **Output Management**: Remote command results are processed with character-based truncation, caching, and formatting.
